...
# Configuration
client_to_filter = "Designs For Health"  # Client specific configuration
markdown_content = "" 
...
def format_df_for_markdown(df_original):
    ...
    ratio_cols = [ # For ROAS and Frequency, typically 2 decimal places
        ...
        'avg_frequency_7day', 'frequency_7day', 'overall_frequency'
    ]
...
def print_and_accumulate_markdown(text, is_header=False, level=2, is_table=False, df=None): 
    ...
    if is_table and df is not None:
        if not df.empty:
            ...
            # Determine column alignments based on original DataFrame dtypes
            alignments = []
            for col_name in df.columns: # Iterate over original DataFrame's columns
                if pd.api.types.is_numeric_dtype(df[col_name]):
                    alignments.append("right")
                else:
                    alignments.append("left")
            
            markdown_content += formatted_df.to_markdown(index=False, colalign=alignments) + "\\n\\n"
        else:
    ...
    if not df_client_offers.empty:
        ...
        for index, row in df_client_offers.iterrows():
            ...
            offer_details_for_header.append({
                ...
                "campaign_count": f"{campaign_count_for_offer:,}" if pd.notnull(campaign_count_for_offer) else "0", 
                "ad_set_count": f"{ad_set_count_for_offer:,}" if pd.notnull(ad_set_count_for_offer) else "0"
            })
    else:
...
try:
    ...
    df_overall_summary = pd.DataFrame() # Ensure it's an empty DataFrame on error for downstream checks
...
try:
    ...
    df_campaign_highlights = pd.DataFrame() # Ensure empty df on error
...
if not df_overall_summary.empty:
    total_clicks_val = df_overall_summary['total_clicks'].iloc[0] if 'total_clicks' in df_overall_summary.columns and not df_overall_summary['total_clicks'].empty and pd.notnull(df_overall_summary['total_clicks'].iloc[0]) else 0
    overall_ctr_val = df_overall_summary['overall_ctr'].iloc[0] if 'overall_ctr' in df_overall_summary.columns and not df_overall_summary['overall_ctr'].empty and pd.notnull(df_overall_summary['overall_ctr'].iloc[0]) else 0
    total_leads_val = df_overall_summary['total_leads'].iloc[0] if 'total_leads' in df_overall_summary.columns and not df_overall_summary['total_leads'].empty and pd.notnull(df_overall_summary['total_leads'].iloc[0]) else 0
    overall_cpl_val = df_overall_summary['overall_cpl'].iloc[0] if 'overall_cpl' in df_overall_summary.columns and not df_overall_summary['overall_cpl'].empty and pd.notnull(df_overall_summary['overall_cpl'].iloc[0]) else 0
    
    engagement_text = (
        f"Total Clicks: {total_clicks_val:,.0f}\\n"
        f"Overall Average CTR: {overall_ctr_val:.2%}\\n"
        f"Total Leads Captured: {total_leads_val:,.0f}\\n"
        f"Overall Average Cost Per Lead (CPL): ${overall_cpl_val:,.2f}"
    )
...
try:
    ...
    if not df_highest_ctr.empty:
        campaign_name_val = df_highest_ctr['campaign_name'].iloc[0]
        campaign_ctr_val = df_highest_ctr['campaign_ctr'].iloc[0]
        ctr_text = (
            f"- Campaign: {campaign_name_val}\\n"
            f"- CTR: {campaign_ctr_val:.2%}"
        )
...
try:
    ...
    if not df_lowest_cpl.empty:
        campaign_name_val = df_lowest_cpl['campaign_name'].iloc[0]
        campaign_cpl_val = df_lowest_cpl['campaign_cpl'].iloc[0]
        cpl_text = (
            f"- Campaign: {campaign_name_val}\\n"
            f"- CPL: ${campaign_cpl_val:,.2f}"
        )
...
for cat_name, keywords in categories_to_analyze.items():
    ...
    try:
        ...
        if not df_cat.empty and pd.notna(df_cat['total_spend'].iloc[0]) and df_cat['total_spend'].iloc[0] > 0:
            ...
            all_category_dfs.append(df_cat) # Append the original unformatted df for potential concat
...
try:
    ...
    if not df_campaign_metrics_3day.empty:
        ...
        overall_spend_3day_val = df_campaign_metrics_3day['spend_3day'].sum()
        overall_revenue_3day_val = df_campaign_metrics_3day['revenue_3day'].sum()
        overall_purchases_3day_val = df_campaign_metrics_3day['purchases_3day'].sum()
        overall_roas_3day_val = overall_revenue_3day_val / overall_spend_3day_val if overall_spend_3day_val else 0
        overall_cpp_3day_val = overall_spend_3day_val / overall_purchases_3day_val if overall_purchases_3day_val else 0
        
        print_and_accumulate_markdown("Overall Portfolio Performance (Last 3 Days, Calculated from View Data):", is_header=True, level=4)
        summary_3day_text = (
            f"Total Spend: ${overall_spend_3day_val:,.2f}\\n"
            f"Total Revenue: ${overall_revenue_3day_val:,.2f}\\n"
            f"Total Purchases: {overall_purchases_3day_val:,.0f}\\n"
            f"Overall ROAS: {overall_roas_3day_val:.2f}\\n"
            f"Average CPP: ${overall_cpp_3day_val:,.2f}"
        )
...
try:
    ...
    if not df_ad_set_metrics_7day.empty:
        ...
        total_adset_spend_7day_val = df_ad_set_metrics_7day['spend_7day'].sum()
        total_adset_purchases_7day_val = df_ad_set_metrics_7day['purchases_7day'].sum()
        total_adset_impressions_7day_val = df_ad_set_metrics_7day['impressions_7day'].sum()
        total_adset_clicks_7day_val = df_ad_set_metrics_7day['clicks_7day'].sum()
        
        avg_cpp_7day_val = total_adset_spend_7day_val / total_adset_purchases_7day_val if total_adset_purchases_7day_val else 0
        avg_cpm_7day_val = (total_adset_spend_7day_val / total_adset_impressions_7day_val) * 1000 if total_adset_impressions_7day_val else 0
        avg_cpc_7day_val = total_adset_spend_7day_val / total_adset_clicks_7day_val if total_adset_clicks_7day_val else 0
        avg_ctr_7day_val = total_adset_clicks_7day_val / total_adset_impressions_7day_val if total_adset_impressions_7day_val else 0
        ...
        ad_set_efficiency_text = (
            f"Average CPP (Cost Per Purchase): ${avg_cpp_7day_val:,.2f}\\n"
            f"Average CPM (Cost Per Mille): ${avg_cpm_7day_val:,.2f}\\n"
            f"Average CPC (Cost Per Click): ${avg_cpc_7day_val:,.2f}\\n"
            f"Average CTR (Click-Through Rate): {avg_ctr_7day_val:.2%}"
        )
...
try:
    ...
    if not df_ad_set_rf_7day.empty:
        ...
        overall_impressions_7day_rf_val = df_ad_set_rf_7day['total_impressions_7day'].sum()
        overall_reach_7day_rf_val = df_ad_set_rf_7day['total_reach_7day'].sum()
        avg_frequency_7day_val = overall_impressions_7day_rf_val / overall_reach_7day_rf_val if overall_reach_7day_rf_val else 0
        print_and_accumulate_markdown("Overall Average Ad Set Frequency (Last 7 Days, from Base Table):", is_header=True, level=5)
        print_and_accumulate_markdown(f"Average Frequency: {avg_frequency_7day_val:.2f}")
...
if not df_overall_summary.empty:
    overall_roas_7d_val = df_overall_summary['overall_roas'].iloc[0] if 'overall_roas' in df_overall_summary.columns and not df_overall_summary['overall_roas'].empty and pd.notnull(df_overall_summary['overall_roas'].iloc[0]) else 0
    total_spend_7d_val = df_overall_summary['total_spend'].iloc[0] if 'total_spend' in df_overall_summary.columns and not df_overall_summary['total_spend'].empty and pd.notnull(df_overall_summary['total_spend'].iloc[0]) else 0
    total_revenue_7d_val = df_overall_summary['total_revenue'].iloc[0] if 'total_revenue' in df_overall_summary.columns and not df_overall_summary['total_revenue'].empty and pd.notnull(df_overall_summary['total_revenue'].iloc[0]) else 0
    total_purchases_7d_val = df_overall_summary['total_purchases'].iloc[0] if 'total_purchases' in df_overall_summary.columns and not df_overall_summary['total_purchases'].empty and pd.notnull(df_overall_summary['total_purchases'].iloc[0]) else 0
    total_leads_7d_val = df_overall_summary['total_leads'].iloc[0] if 'total_leads' in df_overall_summary.columns and not df_overall_summary['total_leads'].empty and pd.notnull(df_overall_summary['total_leads'].iloc[0]) else 0
    overall_cpl_7d_val = df_overall_summary['overall_cpl'].iloc[0] if 'overall_cpl' in df_overall_summary.columns and not df_overall_summary['overall_cpl'].empty and pd.notnull(df_overall_summary['overall_cpl'].iloc[0]) else 0
    
    overview_text = (
        f"Overall ROAS: {overall_roas_7d_val:.2f}\\n"
        f"Total Ad Spend: ${total_spend_7d_val:,.2f}\\n"
        f"Total Revenue: ${total_revenue_7d_val:,.2f}\\n"
        f"Total Purchases: {total_purchases_7d_val:,.0f}\\n"
        f"Total Leads: {total_leads_7d_val:,.0f}\\n"
        f"Average CPL: ${overall_cpl_7d_val:,.2f}"
    )
    print_and_accumulate_markdown(overview_text)
    
    if not df_campaign_highlights.empty and 'campaign_roas' in df_campaign_highlights.columns and pd.notnull(df_campaign_highlights['campaign_roas'].iloc[0]):
        top_campaign_roas_val = df_campaign_highlights['campaign_roas'].iloc[0]
        top_campaign_name_val = df_campaign_highlights['campaign_name'].iloc[0]
        print_and_accumulate_markdown(f"Top Campaign ROAS (from View data): {float(top_campaign_roas_val):.2f} (from campaign: {top_campaign_name_val})")
...
if not df_campaign_highlights.empty:
    ...
    if top_n_campaigns > 0:
        df_top_campaigns_md = df_campaign_highlights.head(top_n_campaigns) 
...
# 3. Execute: python notebooks/eda_report_views_designs_for_health.py
